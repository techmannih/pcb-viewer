...
export const ToolbarOverlay = ({ children, elements }: Props) => {
  ...
  const availableCopperLayers = useMemo(() => {
    ...
    const innerLayerCount = Math.max(0, Math.min(layerCount - 2, INNER_LAYER_NAMES.length))
  ...
  useEffect(() => {
    if (COPPER_LAYER_SET.has(selectedLayer) && !availableCopperLayerSet.has(selectedLayer)) {
      const fallbackLayer =
